...
class AdminTimeTableController extends Controller
{
    ...
    //     $timetables = TimeTable::whereBetween('start_date', [$start, $end])
    //         ->orWhereBetween('end_date', [$start, $end])
    //         ->with(['course', 'teacher', 'department'])
    //         ->get();

    //     $events = $timetables->map(function ($timetable) {
    //         $startDateTime = Carbon::parse($timetable->start_date)->setTimeFromTimeString($timetable->start_time);
    //         $endDateTime = Carbon::parse($timetable->start_date)->setTimeFromTimeString($timetable->end_time);

    //         return [
    //             'id' => $timetable->id,
    //             'title' => $timetable->course->code . ' - ' . $timetable->teacher->name,
    ...
    public function getCalendarData(Request $request)
    {
        ...
        foreach ($timetables as $timetable) {
            ...
            while ($currentDate <= $endDate) {
                if ($currentDate->dayOfWeek === $timetable->day_of_week) {
                    $events[] = [
                        'id' => $timetable->id,
                        'title' => $timetable->course->code . ' - ' . $timetable->teacher->name,
                        'start' => $currentDate->format('Y-m-d') . ' ' . $timetable->start_time->format('H:i:s'),
                        'end' => $currentDate->format('Y-m-d') . ' ' . $timetable->end_time->format('H:i:s'),
                        'color' => $this->getColorForDepartment($timetable->department_id),
                        'textColor' => '#ffffff',
                        'extendedProps' => [
                            'department' => $timetable->department->name,
