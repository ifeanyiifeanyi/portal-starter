...
<div class="container-fluid">
    <div class="sticky-header p-3">
        ...
        <h3>Course Assignments for <br> <span class="lead" style="color: rgb(84, 5, 104)">{{ $semester->name }}</span></h3>
        ...
        <form action="{{ route('course-assignments.show', $semester->id) }}" method="GET" class="mb-4">
            <div class="row">
                <div class="col-md-4 mb-2">
                    <input type="text" class="form-control" placeholder="Search courses..." name="search" value="{{ $search ?? '' }}">
                </div>
                <div class="col-md-3 mb-2">
                    <select name="department" class="form-control">
                        <option value="">All Departments</option>
                        @foreach($departments as $dept)
                            <option value="{{ $dept->id }}" {{ $filterDepartment == $dept->id ? 'selected' : '' }}>{{ $dept->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3 mb-2">
                    <select name="level" class="form-control">
                        ...
                        @foreach($levels as $lvl)
                            <option value="{{ $lvl }}" {{ $filterLevel == $lvl ? 'selected' : '' }}>Level {{ $lvl }}</option>
                        @endforeach
                    </select>
                </div>
